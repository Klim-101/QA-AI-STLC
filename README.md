...
| **Generates Playwright tests**, for the UI and for the API from your OpenAPI contract, that are verified by execution before they are kept and run in CI without any model.                                                                                                                                                                        | Planned                                               | Phase 3; the generation contract is defined (ADR-0010) but the generator itself has not started yet.                                                                |
...
| **Renders reports** (`qa report` / `qa.report`): a run summary and the requirement → case → result → evidence traceability matrix, as Markdown or HTML, from canonical JSON already recorded under `.qa/` — no hand-written report path exists.                                                                                                    | Available                                             | Phase 3.                                                                                                                                                            |
