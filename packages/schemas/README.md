...
evidence, defect draft, RCA, selector registry, API surface, approval ledger, manifest and
pipeline state. This is the versioned contract between the agent layer and the engine — every
other package derives its types from `z.infer` here rather than declaring a parallel interface.
