...
schema-validated entry instead of hand-editing `config.yaml`. `qa approve <gate> --artifact <path>
...
exits non-zero only when an approved artifact no longer matches its recorded hash. Every command
supports `--json` for machine-readable output and exits non-zero on failure.
