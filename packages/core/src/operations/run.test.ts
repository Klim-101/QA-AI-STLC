...
import type { Runner, RunnerInput } from '../runner.js';
...
function createFakeRunner(handler: (input: RunnerInput) => readonly RunResult[]): FakeRunner {
...
describe('runTestRun', () => {
  it('runs a spec set and persists a run record with every result', async () => {
    ...
    const { runner } = createFakeRunner(() => [
      fakeResult({ id: 'run-result-id-2', status: 'passed' }),
      fakeResult({ id: 'run-result-id-3', status: 'failed', failure: { message: 'boom' } }),
    ]);
  ...
  it('registers the run record and every result in the manifest', async () => {
    const { context, fs } = createContext();
    const { runner } = createFakeRunner(() => [fakeResult({ id: 'run-result-id-2', status: 'passed' })]);
