...
// any of the three, and the demo app (the only fixture this operation is validated against) is
// server-rendered EJS.
...
export function resolveIdentity(
  ...
  config: Config,
  options: { readonly identity?: string; readonly cdpEndpointUrl?: string },
...
/**
 * Writes a fresh (or merged) selector registry, the generated locator module and the
 * missing-test-id report, and registers all three in the manifest. Exported so a CLI-only
 * pick-mode session — which gathers `elements` interactively and is not exposed as an MCP tool
 * (AGENTS.md 12.4: safe mode aside, a human physically clicking through a headed browser is not
 * something an agent can drive over stdio) — shares this persistence tail with `runExplore`
 * instead of duplicating it.
 */
export async function persistExploreResult(
  context: EngineContext,
  store: QaStore,
...
/**
 * `qa explore` / MCP `qa_explore` (P2-05): builds the selector registry (development plan section
 * 6.3) by crawling, then merges in static source analysis (`--static`) when asked. `--verify`
 * skips building anything: it re-checks every stored, non-deprecated element's primary candidate
 * against the live page it was found on and fails (`degraded` non-empty) when one no longer
 * resolves as well as it did when last recorded — the signal a stale selector (e.g. a renamed test
 * ID) needs. Manual pick-mode capture is a separate, CLI-only entry point (see
