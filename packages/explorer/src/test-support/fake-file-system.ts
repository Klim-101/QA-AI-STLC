...
/**
 * An in-memory `FileSystem` for unit tests that exercise explorer operations without touching a
 * real disk (AGENTS.md 5.3, 13). Keys are absolute paths as given by the caller; there is no path
 * normalization, matching what the real Node adapter receives from `paths.ts`.
 */
